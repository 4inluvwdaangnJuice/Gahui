...
<br>포스터의 흐름에 따라 아래의 문헌들을 참고해 주시면 감사하겠습니다
...
| 단계 | 조작 | 목적 |
|---|---|---|
| 1 | 분쇄 | 세포벽 파괴로 유리 환원당 노출 |
| 2 | 향류 2단 압착 | 환원당 회수율 극대화 |
| 3 | 2차 압착액 순환 | 1차 세척액으로 재사용, 공정수 절감 |
| 4 | 미세 고형분 여과 | 후단 여과 폐색·침전 오염 방지 |
...
포스터 그래프의 수치는 아래와 같은 과정으로 산출되었습니다.

```
[ 요구 ]  NCM811 = LiNi₀.₈Co₀.₁Mn₀.₁O₂ 를 Li(a)M(b)O(c) 로 두면
          a = 1                      Li 계수
          b = 0.8 + 0.1 + 0.1 = 1    전이금속(Ni·Co·Mn) 계수 합
          c = 2                      O 계수

          전하 중성 조건에서 전이금속 평균 산화수는 +3
          이를 +2 로 환원하는 데 필요한 전자 수
